  ...
  const uploadFiles = async (files: File[]) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const newPaths = data.data.map((r: any) => r.url || r.primary_path)
        console.log('New image paths:', newPaths)
        const allPaths = [...uploadedPaths, ...newPaths]
        setUploadedPaths(allPaths)
        onImagesUploaded(allPaths)
        console.log('Successfully uploaded and saved image URLs')
      } else {
      ...
      <div className="relative">
        ...
        {!uploading && previews.length < maxFiles && (
          <div className="mt-4 text-center">
            <p className="text-sm text-gray-500">
              Images will be automatically optimized and uploaded
            </p>
          </div>
        )}
